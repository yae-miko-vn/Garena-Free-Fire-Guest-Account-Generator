...
BASE_FOLDER = os.path.join(CURRENT_DIR, "Garena Free Fire Guest Account 🌸")
TOKENS_FOLDER = os.path.join(BASE_FOLDER, "JWT Token 🔑")
ACCOUNTS_FOLDER = os.path.join(BASE_FOLDER, "Guest Accounts 🌸")
RARE_ACCOUNTS_FOLDER = os.path.join(BASE_FOLDER, "Rare Guest Accounts ❤")
COUPLES_ACCOUNTS_FOLDER = os.path.join(BASE_FOLDER, "Couples Account 💝")
GHOST_FOLDER = os.path.join(BASE_FOLDER, "𝐆𝐇𝐎𝐒𝐓")
GHOST_ACCOUNTS_FOLDER = os.path.join(GHOST_FOLDER, "Guest Accounts 🌸")
GHOST_RARE_FOLDER = os.path.join(GHOST_FOLDER, "Rare Guest Accounts ❤")
GHOST_COUPLES_FOLDER = os.path.join(GHOST_FOLDER, "Couples Account 💝")
...
def save_rare_account(account_data, rarity_type, reason, rarity_score, is_ghost=False):
    try:
        ...
        rare_entry = {
            ...
            'region': "Yae-Miko" if is_ghost else account_data.get('region', 'UNKNOWN'),
...
def save_couples_account(account1, account2, reason, is_ghost=False):
    try:
        ...
        couples_entry = {
            ...
            'reason': reason,
            'region': "Yae-Miko" if is_ghost else account1.get('region', 'UNKNOWN'),
...
def generate_custom_password(prefix):
    ...
    return f"{prefix}_ACCOUNT_{random_part2}"
...
def save_jwt_token(account_data, jwt_token, region, is_ghost=False):
    try:
        ...
        token_entry = {
            ...
            'region': "𝐁𝐈𝐒𝐇𝐀𝐋" if is_ghost else region,
            'thread_id': account_data.get('thread_id', 'N/A')
        }
    ...
    while True:
        try:
            ...
            if choice == "00":
                ...
                print(f"\n{get_random_color()}{Colors.BRIGHT}👋 Thank you for using Garena Free Fire Guest Account Generator!{Colors.RESET}")
                sys.exit(0)
            elif choice.isdigit():
...
def about_section():
    ...
    print(f"{get_random_color()}{Colors.BRIGHT}ℹ️  About Account Generator{Colors.RESET}")
...
def main_menu():
    while True:
        ...
        print(f"{get_random_color()}Discord Server: https://discord.gg/f2Xrk6aH8N{Colors.RESET}")
